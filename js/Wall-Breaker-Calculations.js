//////Wall Breaker Calculations

var Set_Wall_Breaker_Values = function(){

	set_wall_breaker_lvl();
	calculate_wall_breaker_cost();
	calculate_wall_breaker_training_time();
}

var calculate_wall_breaker_training_time = function(){

	////wall_breaker training time per in seconds
	var time_per = 120;
	////number of wall_breakers user has inputed 
	var $quantity = $('#Wall_Breaker_quantity').val();
	
	///////////
	//calculate the secs
	var seconds = '00';
	////wall_breaker training time total secs equals training time per multiplied by amount user inputs
	var total_secs = time_per * $quantity;
	//if wall_breaker training time in total secs - total secs rounded down does not equal zero AND is not lower than 0
	...
	var hours = '00';
	////wall_breaker training time in total seconds for hours variable
	...
	var minutes = '00';
	////wall_breaker training time in total seconds for minutes variable
	...
	$('#Wall_Breaker_trainingTime').text(hours + ':' + minutes + ':' + seconds);
}
...
var calculate_wall_breaker_cost = function(){
	/////wall_breaker_amount
	var $amount = $('#Wall_Breaker_quantity').val();
	/////wall_breaker_level
	var $level = $('#Wall_Breaker_level').val();
	////wall_breakers elixer cost display element
	var $elixir_cost = $('#Wall_Breaker_elixirCost');
	////wall_breaker cost per unit
	var cost_per;
	////wall_breakers cost total which is the amount multiplied by cost per unit
	var cost_total;
...
var set_wall_breaker_lvl = function(){
	////wall_breaker_lvl
	var $lvl = $('#Wall_Breaker_level').val();
	////wall_breakers image element
	var $image = $('#Wall_Breaker_image');

	if( $lvl == "1" ){
		$image.attr('src', 'img/wallBreaker1.png');
	}
	else if( $lvl == "2" ) {
		$image.attr('src', 'img/wallBreaker2.png');
	}
	else if( $lvl == "3" ) {
		$image.attr('src', 'img/wallBreaker3.png');
	}
	else if( $lvl == "4" ) {
		$image.attr('src', 'img/wallBreaker4.png');
	}
	else if( $lvl == "5" ) {
		$image.attr('src', 'img/wallBreaker5.png');
	}
	else if( $lvl == "6" ) {
		$image.attr('src', 'img/wallBreaker6.png');
	}
}

function wallBreakerHandleChange(input) {
